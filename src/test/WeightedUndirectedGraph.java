...
//This class will represent a graph with vertices names 0 to n - 1, and edges between the vertices.
public class WeightedUndirectedGraph {
    ...
    private static class Edge {
        ...
        //returns the other vertex with me in the edge
    ...
    //-----------------------------------------------Q1: Prime Algorithm-----------------------------------------------//
    //this method is prims algorithm to find a MST in this graph and return it
    public WeightedUndirectedGraph prim() {
        ...
        //init the PQ and have it compare by the key values
        PriorityQueue<Integer> pq = new PriorityQueue<>(n,((o1, o2) -> o1.compareTo(o2)));
        for (int i = 0; i < n; ++i) {
            pq.add(i);
        }
        ...
        while (!pq.isEmpty()) {
            ...
            for (Edge edge : _adjacencylist[u]) {
                ...
                if (edge.get_w() < key[v] && pq.contains(v)) {
                    pi[v] = u;
                    key[v] = edge.get_w();
    ...
    //-----------------------------------------------Q2: New MST-----------------------------------------------//
    //this method (activated on a MST graph) will receive a new edge and will update the MST by locating a circle in it and removing the heaviest edge in it
    public void mstNewEdge(Edge e) {
        this.addEdge(e);
        //locate the circle created by the new edge
        LinkedList<Edge> circle = this.findCircle();
        //validity check to get rid of warnings
        if (circle == null) {
            return;
        }
        //finding the heaviest edge
    ...
    private LinkedList<Edge> findCircle() {
        ...
        for (int u = 0; u < n; u++) {
            ...
            if (color[u] == WHITE) {
                ...
                //if a circle found no need to continue, return the circle
                if (circle != null) {
                    return circle;
                }
            }
    ...
    private LinkedList<Edge> dfsVisit(int u, int[] color, Edge[] pi, int[] d, int[] f, AtomicInteger time) {
        ...
        for (Edge edge : _adjacencylist[u]) {
            ...
            //if this edge is a backwards edge (meaning we reached a gray vertex which isn't v's father) find the circle
    ...
    //gets 2 vertices that connect the circle and the pi(fathers) array and returns a list of edges which construct the circle
    private LinkedList<Edge> getCircle(int v, int y, Edge e, Edge[] pi) {
        ...
        int u = y;
        Edge father = pi[u];
        //keep adding the edges by their fathers till we completed the circle
        while (u != v) {
            circle.add(father);
            u = father.getOther(u);
            father = pi[u];
